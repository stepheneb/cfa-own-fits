    ...
    with Image(filename=infile) as img:
        ...
        for r in 1, 0.5, 0.25, 0.125:
            ...
            with img.clone() as i:
                ...
                outpath = f"{outbase}.jpg"
                i.save(filename=outpath)
                for channel in "R", "G", "B":
                    print(channel)
                    pixels = i.export_pixels(channel_map=channel, storage='float')
                    ...
                    # img2 = np.array(img.export_pixels(channel_map='R', storage='float')
                    # img2.reshape((scaled_width, scaled_height))
                    # img2 = img2.reshape((scaled_width, scaled_height))
                    # img2 = img2.astype(np.float32)
                    # check_image(img2)

                    # active_mask = CHANNELS[channel]
                    # outpath = f"{outbase}-{channel}.jpg"
                    # i.save(filename=outpath)


            # i.resize(int(i.width * r * 0.25), int(i.height * r * 0.25))
            # i.rotate(90 * r)
            # i.save(filename=f"{outdir}/base_filename-{format(r)}.png"

for entry in os.scandir(indir):
    if (entry.path.endswith(".jpg") or entry.path.endswith(".JPG")):
        # print(entry.path)
